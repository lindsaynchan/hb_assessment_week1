...
def long_words(words):
    ...
    If no words are longer than 4 characters, return an empty list::
        ...
        []
    """
    longer_than_four_character_words = []
    for word in words:
        if len(word) > 4:
            longer_than_four_character_words.append(word)
    return longer_than_four_character_words
...
def n_long_words(words, n):
    ...
    For example::
        ...
        ['apples', 'bananas']
    """
    longer_than_n_character_words = []
    for word in words:
        if len(word) > n:
            longer_than_n_character_words.append(word)
    return longer_than_n_character_words    
...
def halvesies(numbers):
    ...
    for i in range(len(numbers)):
        whole_num = float(numbers[i])
        numbers[i] = float(whole_num/2.0)
...
def duplicates(items):
    ...
    You should do this without changing the original list::
        ...
        ['apple', 'apple', 'berry']
    """
    single_items_set = set(items)
    single_items_list = list(single_items_set)

    duplicate_list = []

    
    for single_item in single_items_list:     
        count = 0
        for item in items: 
            if single_item == item:
                count += 1
        if count > 1:
            duplicate_list.append(single_item)
    ...
    **DO NOT** use the `list.index()` method.
    ...
    for word in words:
        if letter not in word:
            index_letter_list.append(None)
        else:
            for i in range(len(word)):
                if word[i] == letter:
                    index_letter_list.append(i)
    print index_letter_list
